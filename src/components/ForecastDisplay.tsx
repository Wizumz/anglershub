...
const getMoonPhase = (date: Date): number => {
  // Simplified moon phase calculation based on astronomical algorithms
  const year = date.getFullYear();
  const month = date.getMonth() + 1;
  const day = date.getDate();
  
  // Calculate the number of days since a known new moon (January 6, 2000)
  const knownNewMoon = new Date(2000, 0, 6, 18, 14, 0);
  const currentDate = new Date(year, month - 1, day);
  const daysSince = (currentDate.getTime() - knownNewMoon.getTime()) / (1000 * 60 * 60 * 24);
              ...
              {weatherInfo && weatherData && (
                <div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                        ...
                        {weatherInfo.isToday && (
                          <>
                            ...
                            <div className="flex justify-between">
                              ...
                              <span>{hPaToInHg(weatherData.current.pressure_msl).toFixed(2)} inHg</span>
                            </div>
                          </>
                        )}
                      </div>
                    ...
                    {/* Solunar Times */}
                    <div className="space-y-3">
                      <h5 className="text-terminal-accent text-sm font-medium">Solunar (EST)</h5>
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
